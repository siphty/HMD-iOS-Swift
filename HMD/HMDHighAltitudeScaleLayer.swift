//
//  HMDHighAltitudeScaleLayer.swift
//  HMD
//
...
class HMDHighAltitudeScaleLayer: CALayer {
    ...
    public override init(){
        super.init()
        print("init HMDHighAltitudeScaleLayer")
        ...
        for i in 0 ... scalesCount {
            if i % shortScaleSections == 0 {
                ...
                drawLine(fromPoint: CGPoint(x: 0.0, y: CGFloat(i) * pixelsPerShortScale),
                         ...
                         width: 1)
            } else if i < ((longScaleSections - 1) * shortScaleSections) && i > (1 * shortScaleSections){
