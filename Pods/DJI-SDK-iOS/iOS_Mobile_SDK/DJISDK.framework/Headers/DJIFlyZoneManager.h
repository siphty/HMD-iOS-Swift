...
/**
 *  Reloads the zone groups unlocked by the account from the server. Each zone
 *  group corresponds to one aircraft. After launching, SDK will load the
 *  information  once. It will also trigger reloading of such information when
 *  network connection  state or account state changes. Invoke the method to trigger
 *  the reloading manually.  However, the loaded zone groups will NOT be
 *  synchronized to the aircraft automatically.  User need to manually invoke
 *  `syncUnlockedZoneGroupToAircraftWithCompletion` to  synchronize the unlocked
 *  zone group to the connected aircraft. The loaded zone groups  can be obtained by
 *  `getLoadedUnlockedZoneGroupsWithCompletion`. This feature is not  supported by
 *  Inspire 1 or Phantom 3 series. For the other products, it can be used  when
 ...
 *  connected aircraft. The list contains the authorization fly zones unlocked by
 *  the Flight Planner (http://www.dji.com/flysafe/geo-system#planner), and the
 *  ones unlocked during flight using DJI Go or any DJI Mobile SDK based
 *  application.  For Inspire 1 and Phantom 3 series, the list is fetched from the
 *  server.  If the internet is not accessible, the cached local data will be used.
 *  For the other  products, the list is fetched from the aircraft.
 *  
