...
#import "BinocularVideoPreviewer.h"
...
//gl view
@property (nonatomic, strong) MovieGLView* internalGLViewLeft;
@property (nonatomic, strong) MovieGLView* internalGLViewRight;
//basic status
...
@implementation BinocularVideoPreviewer
...
{
    static BinocularVideoPreviewer* previewer = nil;
    if(previewer == nil)
    {
        @synchronized (self) {
            if (previewer == nil) {
                previewer = [[BinocularVideoPreviewer alloc] init];
    ...
    while(_status.isRunning)
    {
        @autoreleasepool {
            ...
            if (stream_info_changed) {
                __weak BinocularVideoPreviewer* target = self;
                dispatch_async(dispatch_get_main_queue(),
                               ^{
...
-(uint8_t*) testQueuePull:(int*)size{
    ...
    while (g_fp)
    {
        ...
        while (!feof(g_fp)) {
            ...
            //                        [[[BinocularVideoPreviewer instance] dataQueue] push:pBuffer length:nRead];
            read_size += nRead;
